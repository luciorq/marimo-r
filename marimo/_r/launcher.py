...
def _find_workspace_root_cached(current: Path) -> Path | None:
...
def _executable_in_prefix(prefix: Path, name: str) -> Path | None:
...
@lru_cache(maxsize=8)
def _pixi_env_prefix(root: Path, env_name: str) -> Path | None:
...
def find_pixi_r_prefix(start: Path | None = None) -> Path | None:
...
def find_pixi_r_binary(start: Path | None = None) -> Path | None:
...
def find_r_tool(name: str, start: Path | None = None) -> str | None:
...
def _library_for(binary: Path) -> str | None:
...
def build_environment(
    binary: Path, base_env: dict[str, str] | None = None
